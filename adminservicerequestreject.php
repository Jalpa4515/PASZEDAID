...
/* $results =  $wpdb->get_results("SELECT s.*, sc.name FROM $tabel_name3 as s LEFT JOIN wp_service_category_relation as scr ON s.id = scr.service_id LEFT JOIN wp_service_categories as sc ON scr.category_id = sc.id WHERE s.id = $id");
