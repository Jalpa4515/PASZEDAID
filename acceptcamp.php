...
$subjectnn = "ZED$id - $campt - $fundtitle - $res->address - Has been approved by ZedAid, you can track the progress at : $shorturl";

$to = $user_email;
$subject = "ZED$id - $campt - $fundtitle - $address - Has been approved by ZedAid";
...
if($campTypeId == 1) {
    $file = 'email/fundraiser_admin_approved.html';    
} else {
    $file = 'email/camp_admin_approved.html';    
}
...
// $message .= '<p style="font-size:18px;">Your campaign <b>' . $fundtitle . '</b> has beed approved by admin.</p>';
...
$sql21 = $wpdb->prepare(
    "UPDATE `wp_campaigns` SET `admin_approved` = 1 WHERE id = " . $id
);
$wpdb->query($sql21);
