...
//admin menu code
add_action('admin_menu', 'services');
function services()
{
...
function services_manage_view()
{
	...
	if (!empty($_GET['s']) && !empty($_GET['categoryId'])) {
		...
		$allcoupons = $wpdb->get_results("SELECT s.*, s.id, sc.name FROM $tabel_name3 as s LEFT JOIN wp_service_category_relation as scr ON s.id = scr.service_id LEFT JOIN wp_service_categories as sc ON scr.category_id = sc.id WHERE (s.service_name LIKE '%" . $s . "%' OR s.description LIKE '%" . $s . "%') AND s.is_draft = 0  AND scr.category_id IN ($categoryId) GROUP BY s.id order by " . $orderby . " " . $order . " LIMIT $offset, $no_of_records_per_page");

		$resultscount =  $wpdb->get_results("SELECT * FROM $tabel_name3 as s LEFT JOIN wp_service_category_relation as scr ON s.id = scr.service_id LEFT JOIN wp_service_categories as sc ON scr.category_id = sc.id WHERE (s.service_name LIKE '%" . $s . "%' OR s.description LIKE '%" . $s . "%') AND s.is_draft = 0  AND scr.category_id IN ($categoryId)GROUP BY s.id");
		
	} else if (!empty($_GET['s'])) {

		$s = $_GET['s'];

		$allcoupons = $wpdb->get_results("SELECT s.*, s.id, sc.name FROM $tabel_name3 as s LEFT JOIN wp_service_category_relation as scr ON s.id = scr.service_id LEFT JOIN wp_service_categories as sc ON scr.category_id = sc.id WHERE (s.service_name LIKE '%" . $s . "%' OR s.description LIKE '%" . $s . "%') AND s.is_draft = 0  GROUP BY s.id order by " . $orderby . " " . $order . " LIMIT $offset, $no_of_records_per_page");

		$resultscount =  $wpdb->get_results("SELECT * FROM $tabel_name3 as s LEFT JOIN wp_service_category_relation as scr ON s.id = scr.service_id LEFT JOIN wp_service_categories as sc ON scr.category_id = sc.id WHERE (s.service_name LIKE '%" . $s . "%' OR s.description LIKE '%" . $s . "%') AND s.is_draft = 0  GROUP BY s.id");

	} else if (!empty($_GET['categoryId'])) {

		$categoryId = $_GET['categoryId'];

		$allcoupons = $wpdb->get_results("SELECT s.*, s.id, sc.name FROM $tabel_name3 as s LEFT JOIN wp_service_category_relation as scr ON s.id = scr.service_id LEFT JOIN wp_service_categories as sc ON scr.category_id = sc.id WHERE scr.category_id IN ($categoryId) AND s.is_draft = 0  GROUP BY s.id order by " . $orderby . " " . $order . " LIMIT $offset, $no_of_records_per_page");

		$resultscount =  $wpdb->get_results("SELECT s.* FROM $tabel_name3 as s LEFT JOIN wp_service_category_relation as scr ON s.id = scr.service_id LEFT JOIN wp_service_categories as sc ON scr.category_id = sc.id WHERE scr.category_id IN ($categoryId) AND s.is_draft = 0  GROUP BY s.id");
		
	} else {
		
		$allcoupons = $wpdb->get_results("SELECT s.*, s.id, sc.name FROM $tabel_name3 as s LEFT JOIN wp_service_category_relation as scr ON s.id = scr.service_id LEFT JOIN wp_service_categories as sc ON scr.category_id = sc.id WHERE s.is_draft = 0  GROUP BY s.id order by " . $orderby . " " . $order . " LIMIT $offset, $no_of_records_per_page");

		$resultscount =  $wpdb->get_results("SELECT s.* FROM $tabel_name3 as s LEFT JOIN wp_service_category_relation as scr ON s.id = scr.service_id LEFT JOIN wp_service_categories as sc ON scr.category_id = sc.id WHERE s.is_draft = 0  GROUP BY s.id");
	...
	<div class="refresh_data">
		...
		<div class="container">
			...
			<h4 style="display:inline; float:left;"><b>Servicecs</b></h4>
			...
			<script>
				...
				function changestatus(id) {
					...
					jQuery.ajax({
						type: "POST",
						url: '../adminservicechangestatus.php',
						data: 'id='+id,
				...
				function requestReject(id) {
					...
					jQuery.ajax({
						type: "POST",
						url: '../adminservicerequestreject.php',
						data: 'id='+id,
						success: function(response)
						{
			...
			<form style="margin-left: 10px;display: inline;float: right;width: 35%;" enctype='multipart/form-data' action="admin.php?page=services-view" method="get">

				<input value="services-view" name="page" type="hidden">
						...
						<tr>
							...
							<td>
								<div id="acceptdivmain<?php echo $data->id; ?>">
									<?php if($zed_verified == 0 && $status != '2'){?>
									...
									<?php } else if($status == '2') { ?>
										<button type="button" class="btn align-middle">Reject</button>
									<?php } else { ?>
...
function services_manage_import()
{
	global $wpdb;
	// echo '<pre>';
	// print_r($_POST);
	...
	if ($file) {
		...
		wp_redirect('https://jedaitestbed.in/zed/wp-admin/admin.php?page=flood-crisis-view');
		exit;
	} else {
		wp_redirect('https://jedaitestbed.in/zed/wp-admin/admin.php?page=flood-crisis-view&file=no');
		exit;
	}
}
...
function my_action_javascript2112()
{ ?>
