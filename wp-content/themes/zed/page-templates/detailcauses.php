...
if(isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on')   
  ...
  if($res->campaign_typeId == 1) {
    $resultsdonacc = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}campaigndonations WHERE status = 1 AND paymetStatus = 1 AND campaign_Id IN(" . $id . ")", ARRAY_A);    
  } else {
  ...
  if ($res->id == 24) {
    ...
    $achieve_amount = 0;
    $achieve_amount_offline = 0;
    $achieve_amount_online = 0;
    foreach ($resultsdonacc as $tt) {
      ...
      if($payment_type == 2){
          if ($tt['campaign_typeId'] == 3) {
              //$achieve_amountn = $tt['amount'] ? $tt['amount'] * ($res->product_price ? $res->product_price : 0) : 0;
              $achieve_amountn_offline = $tt['amount'] ? $tt['amount'] : 0;
          } else {
              $achieve_amountn_offline = $tt['amount'] ? $tt['amount'] : 0;
          }
          $achieve_amount_offline += $achieve_amountn_offline;
      }else{
          if ($tt['campaign_typeId'] == 3) {
              //$achieve_amountn = $tt['amount'] ? $tt['amount'] * ($res->product_price ? $res->product_price : 0) : 0;
              $achieve_amountn_online = $tt['amount'] ? $tt['amount'] : 0;
          } else {
              $achieve_amountn_online = $tt['amount'] ? $tt['amount'] : 0;
          }
          $achieve_amount_online += $achieve_amountn_online;
      }
    }
  ...
  if($campaign_typeId == 1) {
    $allresultsdona = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}campaigndonations WHERE campaign_Id IN(" . $id . ") and status = 1 AND paymetStatus = 1 ORDER BY id DESC ");    

    $allcount = count($allresultsdona);

    $rowperpage = 3;
    $resultsdona = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}campaigndonations WHERE campaign_Id IN(" . $id . ") and status = 1 AND paymetStatus = 1 ORDER BY id DESC LIMIT 0, $rowperpage");

  } else {
    $allresultsdona = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}campaigndonations WHERE campaign_Id IN(" . $id . ") and status = 1 ORDER BY id DESC ");    

    $allcount = count($allresultsdona);

    $rowperpage = 3;
    $resultsdona = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}campaigndonations WHERE campaign_Id IN(" . $id . ") and status = 1 ORDER BY id DESC LIMIT 0, $rowperpage");    
  }
                ...
                <div id="Description">
                  ...
                  <div class="tp-case-content">
                    <div class="tp-case-text-top">
                      ...
                      <ul>
                        <li><span>Raised:</span> <?= $currency; ?> <?php echo $achieve_amount ? number_format(str_ireplace(",","", $achieve_amount)) : 0; ?>
                        <span>(Offline: <?= $currency." ".number_format(str_ireplace(",","", $achieve_amount_offline)); ?>)</span>
                        </li>
                        <?php if($achieve_amount > $goal_amount){?>
                          <li><span>Goal:</span> <?= $currency; ?> <?php echo number_format(str_ireplace(",", "", $achieve_amount)); ?></li>
                        <?php } else { ?>
                        ...
                        <img src="<?php echo bloginfo('template_directory'); ?>/images/online.png" width="30" style="height: 35px;    margin-right: 10px;"><div style="font-size: 16px;"><div>
							 <?= $currency." ".number_format(str_ireplace(",","", $achieve_amount_online)); ?>
							</div>Online Donation</div>
								<img src="<?php echo bloginfo('template_directory'); ?>/images/off.png" style="height: 35px;    margin-left: 40px;" width="30"><div style="font-size: 16px;margin-left:10px"><div><?= $currency." ".number_format(str_ireplace(",","", $achieve_amount_offline)); ?></div>Offline Donation</div>
								
							</div>
							</div> -->
              <br>
						<div class="none" style="margin-top:30px;margin-bottom:20px; display:none;">
								<div style="display:flex">
                  <img src="<?php echo bloginfo('template_directory'); ?>/images/online.png" width="30" style="height: 35px;margin-right: 10px;">
                  ...
                  <br>Online Donation
							...
							<div style="margin-top:7%;">								
								<div style="display:flex">
									<img src="<?php echo bloginfo('template_directory'); ?>/images/off.png" style="height: 35px;margin-right: 10px;" width="30">
								<div style="font-size: 16px;"><?= $currency." ".number_format(str_ireplace(",","", $achieve_amount_offline)); ?><br>Offline Donation</div>
              </div>
              ...
              <div class="p-1 mt-1 mb-3 d-none d-sm-block box-stick__border-light">
                ...
                <div class="h-100 d-inline-block w-100 align-top mt-0 side-t-con text-center ">
                  <h4 class="d-block box-stick__color-grey text-center ">
                    ...
                    <?php if($achieve_amount > $goal_amount){?>
                      <span class="box-stick__color-light">of <?= $currency; ?> <?php echo number_format(str_ireplace(",", "", $achieve_amount)); ?></span>
                    <?php } else { ?>
                ...
                <div class="cause-text">
                  ...
                  <ul>
                    ...
                    <li><a target="_blank" href="https://www.linkedin.com/shareArticle?url=<?= $sharelinnk; ?>" class="linkedin"><i class="ti-linkedin"></i></a></li>
                  </ul>
                </div>
              </div>
              <?php
