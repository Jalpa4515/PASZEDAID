...
//$conn->query("INSERT INTO wp_logs SET module = 'payment-request', response = '".json_encode($_POST)."'");
$conn->query("INSERT INTO wp_logs SET module = 'razorpay payment API response', request='".json_encode($_POST)."', response = '".json_encode($payment->toArray())."'");
...
//$generated_signature = hmac_sha256($_POST['razorpay_order_id'] + "|" + $_POST['razorpay_payment_id'], $keySecret);

$generated_signature = hash_hmac("sha256", $_POST['razorpay_order_id'] . "|" . $_POST['razorpay_payment_id'], $keySecret);
...
$sqllog = "INSERT INTO wp_logs (module,request,response) VALUES ('".$error."','" . $razorpay_payment_id . "','".json_encode($payment->toArray())."')";
$conn->query($sqllog);
...
if ($success === true) {
    ...
    $sql_update = "UPDATE wp_campaigndonations SET invoice_id = '".$invoice_id."', razorpay_payment_id = '".$razorpay_payment_id."', paymetStatus = '".$paymetStatus."' WHERE razorpay_order_id = '".$razorpay_order_id."'";
    $conn->query($sql_update);
    
    session_start();
    $_SESSION['donationId'] = $last_id;

    $sqllog = "INSERT INTO wp_logs (module,request,response) VALUES ('RazorPayment_Query_Inserted','" . $last_id . "','Success')";
    $conn->query($sqllog);
    
    /* } else {
      $sqllog = "INSERT INTO wp_logs (module,request,response) VALUES ('RazorPayment_Query_Inserted','" . $razorpay_payment_id . "','Failure')";
      $conn->query($sqllog);
    } */
   ...
   $sqllog = "INSERT INTO wp_logs (module,request,response) VALUES ('RazorPayment_Email_Admin','" . $fundtitle . "','". $message2 ."')";
    ...
    header("Location: https://zedaid.org/donation-failed/");
}